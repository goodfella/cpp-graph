...
{
    template <class Node_Exps = std::tuple<>, class Rel_Exps = std::tuple<>>
    requires ngmg::cypher::is_node_expression_tuple<Node_Exps>::value && ngmg::cypher::is_relationship_expression_tuple<Rel_Exps>::value
    class create_clause
    {
        public:
    ...
    template<class Node_Exps, class Rel_Exps>
    ...
    create_clause<Node_Exps, Rel_Exps>::write(std::ostream & stream) const
    {
        ...
        {
            std::apply([&stream] (auto ... ne) {ngmg::cypher::write_node_expressions(stream,ne...);}, this->_node_expressions);
        }

        if constexpr (std::tuple_size_v<Rel_Exps> > 0)
        {
            std::apply([&stream] (auto ... re) {ngmg::cypher::write_node_expressions(stream,re...);}, this->_relationship_expressions);
        }
    }
}
